...
# 2nd -----------starting the defining portion---------
...
OUTPUT: Return ONLY the optimized prompt with no preamble, explanation, or meta-commentary."""
    ...
    json_promptimizer = { # Response format
...
async def send_all_models(session, user_input):
    logger.info("Start gather") #logger object, print with properties associated for logging purposes
   
    optimized_prompt = await promptimizer(session, user_input)
 
    send = await asyncio.gather( # Turns these into task objects
    ...
    for i, s in enumerate(send): # Not quite sure, some form of error handling
...
# ACTUALLY RUN FIRST, this sends the post request defining WHICH model we're working with, one at a time within a for loop
    for model_name, model_id in models.items():
        try:
            ...
            async with session.post(ollama_endpoint,json=request_data) as req: # Auto sends the post request via async with
                req.raise_for_status() # Raise for status via http response
                data = await req.json() # await a json response 
               
               
        except Exception as e:
            logger.error(f"Error unloading {model_name}: {e}") # This whole block sends the initial defining request ot the ollama endpoint

#defining what will run, 4th
async def main():
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        while True:
            try:
                ...
                # Only run if exited, User:"exit" -> model_cleanup from memory
                ...
                qwen_small_response, qwen_response, llama_response = await send_all_models(session, user_input) # THIS IS WHAT IS ACTUALLY RUN IF SUCCESSFUL
...
# ----------------Ending the defining portion------------------

# 5th, ACTUALLY RUNNING EVERYTHING, now running portion
